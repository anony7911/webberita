...
<head>
    <meta charset="utf-8">
    <title>{{ $detartikels->judul }} | Kolaka Update</title>
    ...
    <div class="container-fluid py-3">
        <div class="container">
            <div class="row">
                ...
                {{-- content --}}
                <div class="col-lg-5 col-sm-12  order-first order-sm-0 px-1 py-1 mt-0">
                    <div>
                        <div class="mb-3 shadow-sm align-items-center border-top border-primary">
                            <div class=" align-items-start justify-content-between bg-light text-dark py-2 px-3 mb-3">
                                <p class="h3">{{ ucfirst($detartikels->judul)}}</p>
                                <div class="mb-3 mt-1" style="font-size: 13px;">
                                    @foreach ($detartikels->kategoris->take(1) as $kategori)
                                    <a href="{{ url('/'."kategori/".$kategori->slug_kategori) }}">{{ ucfirst($kategori->nama_kategori) }}</a>
                                    <span class="px-1">/</span>
                                    @endforeach
                                    <a href="#">{{ $detartikels->created_at->isoFormat('dddd, D MMMM Y') }}</a>
                                    <p><b>Editor: {{ $detartikels->editor }}</b></p>
                                </div>
                                <img class="img-fluid w-100 justify-content-center mb-3" src="{{ url('/assets/img/artikel/'.$detartikels->gambar_artikel) }}">
                                <p class="h6">{!! $detartikels->isi !!}</p>
                                <p><hr color="orange"></p>
                                   <div>
                                       <span>Tags:</span>
                                    @foreach ($detartikels->tags as $tag)
