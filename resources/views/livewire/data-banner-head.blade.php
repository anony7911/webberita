                ...
                <div class="bg-gradient-warning shadow-primary border-radius-lg pt-4 pb-3 pl-3">
                    <div class="row">
                        <div class="col-6 d-flex align-items-left">
                            <h4 class="text-white text-capitalize ps-3">BANNER ATAS</h4>
                        </div>
            ...
            <div class="card-body px-0 pb-2">
                ...
                <form id="isTambah" class="py-4 px-4 pt-0">
                    ...
                    <div class="input-group input-group-dynamic mb-1">
                        <input wire:model="gambar_bannerhead" type="file" class="form-control" id="gambar_bannerhead"
                            aria-describedby="basic-addon3" placeholder="">
                        @error('gambar_bannerhead') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="input-group input-group-dynamic mb-1">
                        <input wire:model="keterangan_bannerhead" type="text" class="form-control" id="keterangan_bannerhead"
                            aria-describedby="basic-addon3" placeholder="Masukkan keterangan gambar disini...">
                        @error('keterangan_bannerhead') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="mb-1">
                ...
                <form id="isEdit" class="py-4 px-4 pt-0">
                    @csrf
                    <div class="input-group input-group-dynamic mb-1">
                        <input wire:model="gambar_bannerhead" type="file" class="form-control"
                            aria-describedby="basic-addon3" placeholder="">
                        @error('gambar_bannerhead') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="input-group input-group-dynamic mb-1">
                        <input wire:model="keterangan_bannerhead" type="text" class="form-control"
                            aria-describedby="basic-addon3" placeholder="Masukkan keterangan gambar disini...">
                        @error('keterangan_bannerhead') <span class="text-danger">{{ $message }}</span>@enderror
                    </div>
                    <div class="mb-1">
                    ...
                    <div class="col-md-12">
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                ...
                                <tbody>
                                    @forelse($bannerheads as $bannerhead)
                                    <tr >
                                        <td>
                                            <div class="d-flex px-2 py-1">
                                                <div class="d-flex flex-column justify-content-left px-2">
                                                    <img class="mb-0"
                                                        src="{{ Storage::url($bannerhead->gambar_bannerhead) }}"
                                        ...
                                        <td class="flex-column align-middle"style="width:20%">
                                            <p style="width:20%" style="word-break: break-word" class="text-xs font-weight-bold mb-0 w-25 text-break">
                                                {{ $bannerhead->keterangan_bannerhead }}
                                            </p>
                                        </td>

                                        <td class="align-middle text-left  text-sm">
                                            @if($bannerhead->status_bannerhead == 1)
                                            <button wire:click="update_status_off({{ $bannerhead->id }})" class="btn btn-sm btn-success font-weight-bold text-xs mb-0">Aktif</button>
                                            @else
                                            <button wire:click="update_status_on({{ $bannerhead->id }})" class="btn btn-sm btn-danger font-weight-bold text-xs mb-0">Nonaktif</button>
                                            @endif
                                        </td>
                                        <td class="align-middle text-left">
                                            <span
                                                class="text-secondary text-xs font-weight-bold">{{ $bannerhead->updated_at }}</span>
                                        </td>
                                        <td class="align-middle">
                                            <button wire:click="edit({{ $bannerhead->id }})"
                                                class="btn btn-sm btn-info font-weight-bold text-xs mb-0">
                                                Edit
                                            </button>
                                            <button wire:click="delete({{ $bannerhead->id }})"
